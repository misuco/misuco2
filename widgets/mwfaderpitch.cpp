...
void MWFaderPitch::onPressedPitch(int id)
{
    ...
    if(pressed < 2) {
        eventId=id;
        vId=out->noteOn(MisuWidget::channel,_freq->getFreq(),_freq->getMidinote(),_freq->getPitch(),127);
...
void MWFaderPitch::onUpdatedPitch(int id)
{
    ...
    if(id == eventId) {
        MisuWidget::Microtune.tuning[_freq->getRootNote()] = getValue();
        out->pitch(MisuWidget::channel,vId,_freq->getFreq(),_freq->getMidinote(),_freq->getPitch());
    }
}
